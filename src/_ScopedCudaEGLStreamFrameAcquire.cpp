...
namespace ArgusSamples
{
                                                            ...
                                                            float *time)
    {
        ...
        uchar *d_Y;
        uchar *d_CrCb;
        uchar *d_CrCb_interleaved;
        uchar *d_bgr;
        uchar *d_Cr;
        uchar *d_Cb;
        uchar *d_Cr_resized;
        uchar *d_Cb_resized;
        uchar *d_YCrCb_resized;

        uchar *h_Y = (uchar *)malloc(sizeof(uchar) * WIDTH * HEIGHT);
        // uchar *h_CrCb = (uchar *)malloc(sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF);
        // uchar *h_CrCb_interleaved = (uchar *)malloc(sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF);
        ...
        uchar *h_YCrCb_resized = (uchar *)malloc(sizeof(uchar) * WIDTH * (HEIGHT + HEIGHT_HALF));
        uchar *h_BGR = (uchar *)malloc(sizeof(uchar) * WIDTH * HEIGHT * 3);
        ...
        // Width/2, Height/2
        cudaMalloc(&d_CrCb_interleaved, sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF);
        ...
        cudaMalloc(&d_YCrCb_resized, sizeof(uchar) * WIDTH * (HEIGHT + HEIGHT_HALF));
        ...
        err = cudaMemcpy2DFromArray(d_CrCb_interleaved,
        ...
        unravel(d_CrCb_interleaved, d_Cb, d_Cr, WIDTH_HALF, HEIGHT_HALF);
        ...
        err = cudaMalloc(&d_Cr_resized, resize_len * sizeof(uchar));
        checkError(err, "cudaMemcpy - d_Cr_resized");

        err = cudaMalloc(&d_Cb_resized, resize_len * sizeof(uchar));
        checkError(err, "cudaMemcpy - d_Cr_resized");

        NppiSize oSrcSize = {WIDTH_HALF, (HEIGHT_HALF / 2)};
        NppiSize oDstSize = {WIDTH_HALF * nXFactor, (HEIGHT_HALF / 2) * nYFactor};
        NppiRect oSrcROI = {0, 0, WIDTH_HALF, (HEIGHT_HALF / 2)};
        NppiRect oDstROI = {0, 0, (WIDTH_HALF * nXFactor), ((HEIGHT_HALF / 2) * nYFactor)};

        NppStatus nppStatus;

        nppStatus = nppiResize_8u_C1R(d_Cr,
        ...
        nppStatus = nppiResize_8u_C1R(d_Cb,
                                      oSrcSize.width * sizeof(uchar),
                                      oSrcSize,
                                      oSrcROI,
                                      d_Cb_resized,
                                      ...
                                      oDstROI,
                                      NPPI_INTER_NN);
        checkNppStatus(nppStatus, "Resizing Cb plane");

        // // <in progress>
        // uchar *pSrc[] = {d_Y, d_Cr_resized, d_Cb_resized};
        // // uchar *pSrc[] = {d_Y, d_Cr_resized, d_Cb_resized};
        // int rSrcStep[] = {WIDTH * sizeof(uchar),
        //                   WIDTH_HALF * sizeof(uchar),
        //                   WIDTH_HALF * sizeof(uchar)};
        // NppiSize oSizeROI = {WIDTH, HEIGHT};
        // int nDstStep = WIDTH * sizeof(uchar) * 3;
        // // </in progress>

        // // <failed>
        // nppStatus = nppiYUV420ToBGR_8u_P3C3R(pSrc, rSrcStep, d_bgr, nDstStep, oSizeROI);
        // checkNppStatus(nppStatus, "YUV420 to BGR");
        // // </failed> 

        // <failed>
        // nppStatus = nppiYCbCr420ToBGR_8u_P3C3R(pSrc, rSrcStep, d_bgr, nDstStep, oSizeROI);
        // checkNppStatus(nppStatus, "YUV420 to BGR");
        // </failed>
        ...
        // <working>
        uchar *ptr = d_YCrCb_resized;

        err = cudaMemcpy(ptr, d_Y, sizeof(uchar) * WIDTH * HEIGHT, cudaMemcpyDeviceToDevice);
        checkError(err, "cudaMemcpy - Copying d_Y to d_YCrCb_resized");

        ptr += (WIDTH * HEIGHT);

        err = cudaMemcpy(ptr, d_Cr_resized, sizeof(uchar) * WIDTH * HEIGHT_HALF / 2, cudaMemcpyDeviceToDevice);
        checkError(err, "cudaMemcpy - Copying d_Cr to d_YCrCb_resized");

        ptr += (WIDTH * HEIGHT_HALF / 2);

        err = cudaMemcpy(ptr, d_Cb_resized, sizeof(uchar) * WIDTH * HEIGHT_HALF / 2, cudaMemcpyDeviceToDevice);
        checkError(err, "cudaMemcpy - Copying d_Cb to d_YCrCb_resized");
        // </working>
        ...
        // checkError(err, "cudaMemcpy");
        // err = cudaMemcpy(h_CrCb_interleaved, d_CrCb_interleaved, sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF, cudaMemcpyDeviceToHost);
        // checkError(err, "cudaMemcpy");
        ...
        // cv::cuda::GpuMat gpuMatYCrCb, gpuMatBGR;
        // gpuMatYCrCb.create((HEIGHT + HEIGHT_HALF), WIDTH, CV_8U);
        // gpuMatYCrCb.data = d_YCrCb_resized;
        // gpuMatYCrCb.step = WIDTH;

        // // gpuMatBGR.create(HEIGHT, WIDTH, CV_8UC3);
        // // gpuMatBGR.step = WIDTH * 3;


        // cv::cuda::cvtColor(gpuMatYCrCb, gpuMatBGR, cv::COLOR_YCrCb2BGR, 3);
        // // cv::cuda::cvtColor(gpuMatYCrCb, gpuMatBGR, cv::COLOR_YUV420p2BGR, 3);

        // cv::Mat cpuMatBGR;
        // // cpuMatBGR.create(HEIGHT , WIDTH, CV_8UC3);
        // gpuMatYCrCb.download(cpuMatBGR);

        // cv::imshow("cpuMatBGR", cpuMatBGR);
        // cv::waitKey(1);        


        // // <working>
        // err = cudaMemcpy(h_YCrCb_resized, d_YCrCb_resized, sizeof(uchar) * WIDTH * (HEIGHT + HEIGHT_HALF), cudaMemcpyDeviceToHost);
        // checkError(err, "cudaMemcpy");

        // cv::Mat matYCrCb;
        // matYCrCb.create((HEIGHT + HEIGHT_HALF), WIDTH, CV_8U);
        // matYCrCb.data = h_YCrCb_resized;
        // matYCrCb.step = WIDTH;

        // cv::Mat matBGR;
        // matBGR.create(HEIGHT, WIDTH, CV_8UC3);

        // cv::cvtColor(matYCrCb, matBGR, COLOR_YUV420p2BGR);

        // cv::imshow("matBGR",matBGR);
        // cv::waitKey(0);

        // char fileBGR[50];
        // sprintf(fileBGR, "BGR_%d.png", FILE_IDX);

        // cv::imwrite(fileBGR, matBGR);
        // </working>


        // dumpImageFile(h_YCrCb_resized, "h_YCrCb_resized", FILE_IDX, WIDTH, (HEIGHT + HEIGHT_HALF));

        // cv::Mat matY, matCr, matCb, matCrCb;
        // matY.create(HEIGHT, WIDTH, CV_8U);
        // matY.data = h_Y;
        // matY.step = WIDTH;

        // matCrCb.create(HEIGHT_HALF, WIDTH, CV_8U);
        // matCrCb.data = h_Cr;
        // matCrCb.step = WIDTH;
        ...
        // sprintf(fileY, "Y_%d.png", FILE_IDX);
        // sprintf(fileCr, "Cr_%d.png", FILE_IDX);
        // sprintf(fileCb, "Cb_%d.png", FILE_IDX);

        // cv::imshow("Y", matY);
        // cv::imshow("matCrCb", matCrCb);
        // cv::waitKey(1);
        ...
        // dumpImageFile(h_Y, "Y", FILE_IDX, WIDTH, HEIGHT);
        // dumpImageFile(h_CrCb_interleaved, "h_CrCb_interleaved", FILE_IDX, WIDTH_HALF, HEIGHT_HALF);
        // dumpImageFile(h_bgr, WIDTH * HEIGHT * CHANNEL, "bgr", FILE_IDX);
        // dumpImageFile(h_Cr, "Cr", FILE_IDX, WIDTH_HALF / 2, HEIGHT_HALF);
        // dumpImageFile(h_Cb, "Cb", FILE_IDX, WIDTH_HALF / 2, HEIGHT_HALF);
        ...
        // free(h_Y);
        // free(h_CrCb);
        // free(h_CrCb_interleaved);
        free(h_BGR);
        // free(h_Cb);
        free(h_YCrCb_resized);

        cudaFree(d_Y);
        cudaFree(d_CrCb);
        cudaFree(d_CrCb_interleaved);
        cudaFree(d_bgr);
        cudaFree(d_Cr);
        cudaFree(d_Cb);
        cudaFree(d_Cr_resized);
        cudaFree(d_Cb_resized);
