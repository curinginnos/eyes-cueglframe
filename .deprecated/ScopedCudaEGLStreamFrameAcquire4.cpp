...
namespace ArgusSamples
{
                                                            ...
                                                            float *time)
    {
        ...
        // size_t YCrCb_ARRAY_SIZE = (sizeof(uchar) * WIDTH * HEIGHT) + (sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF);
        // uchar *h_YCrCb = (uchar *)malloc(YCrCb_ARRAY_SIZE);
        // uchar *h_YCrCb_middle = &h_YCrCb[WIDTH * HEIGHT];

        // memcpy(h_YCrCb, h_Y, sizeof(uchar) * WIDTH * HEIGHT);
        // memcpy(h_YCrCb_middle, h_CrCb, sizeof(uchar) * WIDTH_HALF * HEIGHT_HALF);

        // cv::Mat cpuMat;
        // cpuMat.create(HEIGHT, WIDTH, CV_8U);
        // cpuMat.data = h_YCrCb;
        // cpuMat.step = WIDTH;
        ...
        // cv::cvtColor(cpuMat, result, COLOR_YUV420sp2BGR);
        ...
        // cv::Mat cpuCrCbMat;
        // cpuCrCbMat.create(HEIGHT_HALF, WIDTH_HALF, CV_8U);
        // cpuCrCbMat.data = h_CrCb;
        // cpuCrCbMat.step = WIDTH_HALF;

        // cv::Mat cpuNV21;
        // cpuNV21.create(HEIGHT, WIDTH, CV_8UC3);
        // cpuNV21.step = WIDTH;
        // cv::Mat cpuNV12;
        // cpuNV12.create(HEIGHT, WIDTH, CV_8UC3);
        // cpuNV12.step = WIDTH;

        // cv::cvtColorTwoPlane(cpuYMat, cpuCrCbMat, cpuNV21, COLOR_YUV2BGR_NV21);
        // cv::cvtColorTwoPlane(cpuYMat, cpuCrCbMat, cpuNV12, COLOR_YUV2BGR_NV12);

        // cv::imshow("Y", cpuYMat);
        // cv::imshow("CrCb", cpuCrCbMat);
        // cv::imshow("NV21", cpuMat);
        // cv::imshow("NV12", result);
        // cv::waitKey(1);
        ...
        cv::Mat matY, matCr, matCb;
        matY.create(HEIGHT, WIDTH, CV_8U);
        matY.data = h_Y;
        matY.step = WIDTH;

        matCr.create(HEIGHT_HALF / 2, WIDTH_HALF, CV_8U);
        matCr.data = h_Cr;

        matCb.create(HEIGHT_HALF / 2, WIDTH_HALF, CV_8U);
        matCr.data = h_Cb;
